  ...
  <nav class="border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex">
          ...
          <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
            <template v-for="item in menuItems">
              <Dropdown v-if="item.subItems ?? false">
                <template #trigger>
                  <button type="button"
                ...
                <template #content>
                  <template v-for="subItem in item.subItems">
                    <DropdownLink :href="route(subItem.route)" :active="route().current(subItem.route)">
                      {{ subItem.label }}
                    </DropdownLink>
                    <hr v-if="subItem.last ?? false"/>
                  </template>
                </template>
              </Dropdown>
              <NavLink v-else :href="route(item.route)" :active="route().current(item.route)">
                {{ item.label }}
              </NavLink>
            </template>
    ...
    >
      <div class="pt-2 pb-3 space-y-1">
        <template v-for="item in menuItems">
          <div v-if="item.subItems ?? false" class="pt-4 pb-1">
            <div class="px-4">
            ...
            <div class="mt-3">
              <template v-for="subItem in item.subItems">
                <ResponsiveNavLink class="ml-0 ps-8"
                                   :class="subItem.last ? 'mb-4' : ''"
